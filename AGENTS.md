...
- **Cross-origin isolation** (non-negotiable): `SharedArrayBuffer` requires `COOP: same-origin`, `COEP: require-corp`, `CORP: same-origin` on every response. Vite sends them in dev (`vite.config.ts`); in prod they are set **by PocketBase itself** via `pb_hooks/main.pb.js` (route-wide header hook + disabled read/write timeouts) — no reverse proxy is needed in front of PocketBase anymore (the k8s-side proxy has been removed; Traefik terminates TLS only). Verify `self.crossOriginIsolated === true` in the browser.
...
- The deploy artifact is a **custom Docker image**: PocketBase (base image `adrianmusante/pocketbase`, pinned) with the built SPA baked in (`dist/` → `/pocketbase/public/`) plus `pb_migrations/` and `pb_hooks/`. Pushing a `v*` git tag triggers the release workflow, which builds the SPA, builds/pushes `harbor.alexklingenbeck.de/my/psflix:<version>` to Harbor (repo secrets `HARBOR_USERNAME` / `HARBOR_PASSWORD`, the `robot$docker` robot account), and the Flux cluster runs the image. Local `docker build` is optional.
- The SPA is served by the same PocketBase that serves the API (same origin), so **no CORS configuration is needed**. Revisit only if the SPA is ever embedded cross-origin.
- A freshly migrated instance has **schema but no content** (no games/discs/BIOS) — catalog data is uploaded via the admin UI or restored from a backup; migrations do not seed it.
