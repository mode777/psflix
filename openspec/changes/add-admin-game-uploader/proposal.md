...
- **External dependency:** Reads from the public psxdatacenter API (`https://mode777.github.io/psxdatacenter-dump/api/<id>.json`) and downloads cover/screenshot image URLs — same sources the CLI uses. Image downloads are cross-origin; they are fetched and re-uploaded as `File` objects (no hot-linking).
