...
- **[Cross-origin image fetch (cover/screenshots) may be blocked by CORS]** → If the image host sends no CORS headers, `fetch` of the image bytes fails. Mitigation: a failed image download degrades gracefully (the game is created without that image), exactly as the CLI already tolerates per-image failure. No correctness impact on the record.
