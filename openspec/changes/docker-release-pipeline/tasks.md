...
- [ ] 5.2 Release dry run on a real tag: push a `v*` tag, confirm the workflow publishes `harbor.alexklingenbeck.de/my/psflix:<version>` (first tag is the cutover validation; requires the image repo to auto-create in Harbor)
- [ ] 5.3 Pull the published image, run with `-p 8090:8090` against a scratch volume, and verify: SPA loads at `/`, migrations applied (collections listed in `/_/`), COI headers present on `/`
