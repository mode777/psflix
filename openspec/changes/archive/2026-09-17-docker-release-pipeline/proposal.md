...
- Change the SPA backend URL default (in `src/lib/pb.ts` and `src/admin/lib/pb.ts`): production builds fall back to same-origin (`window.location.origin`), dev falls back to `http://127.0.0.1:8090`; an explicit `VITE_PB_URL` always wins. **BREAKING** for anyone relying on the previous hardcoded `https://pb.example.com` fallback in a production build — that fallback moves to dev only.
